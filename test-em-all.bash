...
ACCESS_TOKEN=$(curl --location --request POST https://$HOST:$PORT/oauth2/login --header 'Content-Type: application/json' --data-raw '{"usernameOrEmailOrPhone":"admin","password":"123456"}' -ks | jq .access_token -r)
...
READER_ACCESS_TOKEN=$(curl --location --request POST https://$HOST:$PORT/oauth2/login --header 'Content-Type: application/json' --data-raw '{"usernameOrEmailOrPhone":"guest","password":"123456"}' -ks | jq .access_token -r)
